...
if uploaded_file is not None:  # BytesIO 객체
    ...
    df = pd.read_csv(
        StringIO(result_string)
    )  # 문자열을 다시 스트림화해서 read_csv에 넣어줌
    df["사용일자"] = pd.to_datetime(
        df["사용일자"], format="%Y%m%d"
    )  # 사용일자 컬럼값들을 datetime 타입으로 변경
    df["등록일자"] = pd.to_datetime(
        df["등록일자"], format="%Y%m%d"
    )  # 등록일자 컬럼값들을 datetime 타입으로 변경
    st.write(df)
    ...
    first_line = (
        df[df["노선명"] == "1호선"]
        .groupby("역명", as_index=False)["하차총승객수"]
        .mean()
